...
`backend/pyproject.toml` is the single editable dependency source. `backend/requirements.txt` is the
runtime lock and `backend/requirements-dev.txt` is the runtime-plus-development lock. Regenerate them
from `backend/` with
...
pyproject.toml`; never hand-edit either generated file.
