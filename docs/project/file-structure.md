...
`backend/pyproject.toml` is the single editable source of dependency and tool configuration. `backend/requirements.txt` is the generated runtime lock retained for the frozen §14 container/deployment layout; `backend/requirements-dev.txt` is the generated runtime-plus-development lock used by CI. Both contain hashes and pip-compile's native autogenerated header. Regenerate them with the documented `--allow-unsafe --generate-hashes` commands; never hand-edit either or maintain an independent dependency list. P1-07 CI rejects source/lock divergence.

P1-07 adds `scripts/validate_migrations.py` as the strict pre-P2-01 negative migration invariant and `tests/hooks/test_ci_workflow.py` as the executable CI contract. The first P2-01 migration must replace the negative validator with real PostgreSQL upgrade, downgrade, clean-replay, and drift validation.
